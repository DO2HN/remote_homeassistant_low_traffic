...
class RemoteConnection:
	...
	async def _init(self):
		...
		def entities_changed(message):
			"""Handle compressed subscribe_entities updates."""
			...
			updates = message.get("event", {})

			# New / initial entities
			for entity_id, compressed in updates.get("a", {}).items():
				state = compressed.get("s", "unknown")
				attributes = compressed.get("a", {}).copy()
				...
				state_changed(entity_id, state, attributes.copy())

			# Changed entities
			for entity_id, diff in updates.get("c", {}).items():
				cached = entity_cache.get(entity_id)

				if cached is None:
					_LOGGER.warning(
						"Received update for unknown remote entity %s",
						entity_id,
					)
					continue
				...
				if "s" in added:
					cached["state"] = added["s"]

				if "a" in added:
					cached["attributes"].update(added["a"])

				for attr_name in removed.get("a", []):
					cached["attributes"].pop(attr_name, None)

				state_changed(
					entity_id,
					cached["state"],
					cached["attributes"].copy(),
				)

			# Removed entities
			for entity_id in updates.get("r", []):
				entity_cache.pop(entity_id, None)

				local_entity_id = self._prefixed_entity_id(entity_id)

				with suppress(ValueError, AttributeError, KeyError):
					self._entities.remove(local_entity_id)

				with suppress(ValueError, AttributeError, KeyError):
					self._all_entity_names.remove(entity_id)

				self._hass.states.async_remove(local_entity_id)
